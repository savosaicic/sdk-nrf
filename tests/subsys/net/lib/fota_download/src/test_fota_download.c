...
/* Highest MCUboot image pair index accepted by this build. Without the MCUboot
 * dfu_target there is no slot table to protect, so any non-negative value is valid.
 */
...
ZTEST(fota_download_util, test_download_util_img_num)
{
	if (IMG_NUM_MAX < 1) {
		ztest_test_skip();
	}
	...
	struct downloader_evt evt = {
		...
		.fragment = {
			.buf = fragment_buf,
			.len = sizeof(fragment_buf),
		}
	};

	int err = fota_download_util_download_start("http://host.com/file.bin",
				      DFU_TARGET_IMAGE_TYPE_ANY, 1, 0,
				      client_callback);

	zassert_ok(err);

	/* The download is started from a work item */
	k_sleep(K_MSEC(100));

	err = downloader_event_handler(&evt);
	zassert_ok(err);
	zassert_equal(dfu_target_init_img_num, 1, "util passed img_num %d",
		      dfu_target_init_img_num);
	...
	zassert_ok(err);
	if (IS_ENABLED(CONFIG_FOTA_CLIENT_AUTOSCHEDULE_UPDATE)) {
		zassert_equal(dfu_target_schedule_img_num, 1, NULL);
	}

	k_sem_take(&stop_sem, K_FOREVER);
}

ZTEST(fota_download_util, test_download_util_img_num_negative)
{
	init();
	...
	zassert_equal(err, -EINVAL, "%d", err);
}

ZTEST(fota_download_util, test_download_util_schedule_img_num)
{
	init();

	/* fota_download_util_image_schedule() is a no-op with autoscheduling, and needs
	 * the MCUboot dfu_target to pre-initialize the target.
	 */
	if (IS_ENABLED(CONFIG_FOTA_CLIENT_AUTOSCHEDULE_UPDATE) ||
	    !IS_ENABLED(CONFIG_DFU_TARGET_MCUBOOT)) {
		ztest_test_skip();
	}

	int err = fota_download_util_image_schedule(DFU_TARGET_IMAGE_TYPE_MCUBOOT, 1);

	zassert_ok(err);
	zassert_equal(dfu_target_init_img_num, 1, NULL);
	zassert_equal(dfu_target_schedule_img_num, 1, NULL);

	err = fota_download_util_image_schedule(DFU_TARGET_IMAGE_TYPE_MCUBOOT, -1);
	zassert_equal(err, -EINVAL, "%d", err);
}
