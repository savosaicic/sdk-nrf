 ...
 *
 * Zero-initialize the structure and fill in the fields that are needed.
 * A zero-initialized structure with only @c host and @c file set is
 * equivalent to fota_download_any().
 */
struct fota_download_params {
	/** Name of host to download from. Can include scheme and port number,
	 *  for example https://google.com:443. Not copied, must stay valid until
	 *  the download is finished.
	 */
	const char *host;
	/** Path to the file to download. See fota_download_any() for details on
	 *  the expected format. Not copied, must stay valid until the download is finished.
	 */
	const char *file;
	...
	/** Expected type of the firmware file, or 0 for @c DFU_TARGET_IMAGE_TYPE_ANY. */
	 ...
	 *  Must be lower than @kconfig{CONFIG_UPDATEABLE_IMAGE_NUMBER}.
	 *  Ignored for modem image types. For @c DFU_TARGET_IMAGE_TYPE_SMP it selects
	 *  the SMP server image number to upload to.
	 */
	int img_num;
	/** Optional transport-specific host configuration (proxy URI, connection ID,
	 *  authentication callback), or NULL. Fields set elsewhere (interface name,
	 *  native TLS) are preserved.
	 */
 ...
 *
 * Superset of the other start functions. Use this variant to select an MCUboot
 * image pair other than the main application when MCUboot is built with more
 * than one updateable image pair.
 *
 * Validate that the file type matches the expected type before proceeding with the download.
